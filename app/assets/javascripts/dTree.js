    ...
    _createClass(TreeBuilder, [{
      ...
      value: function _update(source) {
        ...
        nodes.append('foreignObject').filter(function (d) {
          ...
          return opts.callbacks.nodeRenderer(d.data.name, d.x, d.y, nodeSize[0], nodeSize[1], d.data.extra, d.data.id, d.data['class'], d.data.textClass, opts.callbacks.textRenderer,d.depth);
        }).on('click', function (d) {
          if (d.data.hidden) {
            return;
          }
          opts.callbacks.nodeClick(d.data.name, d.data.extra, d.data.id, d.depth);
